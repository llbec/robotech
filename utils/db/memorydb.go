...
// MemoryDB is an ephemeral key-value store. Apart from basic data storage
...
// NewMemoryDB returns a wrapped map with all the required database interface methods
// implemented.
...
func NewWithCap(size int) *MemoryDB {
	return &MemoryDB{
...
// not exist).
func (db *MemoryDB) NewIteratorWithStart(start []byte) Iterator {
	db.lock.RLock()
	defer db.lock.RUnlock()
...
func (db *MemoryDB) Stat(property string) (string, error) {
	return "", errors.New("unknown property")
}

// Compact is not supported on a memory database.
func (db *MemoryDB) Compact(start []byte, limit []byte) error {
...
// memdbBatch is a write-only memory batch that commits changes to its host
...
func (b *memdbBatch) ValueSize() int {
	return b.size
}
...
func (b *memdbBatch) Replay(w KeyValueWriter) error {
	for _, keyvalue := range b.writes {
		if keyvalue.delete {
...
// memdbIterator can walk over the (potentially partial) keyspace of a memory key
