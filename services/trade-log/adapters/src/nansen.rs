...
use trade_log::{import_job::TradeSource, NansenPage, RawTrade};
...
#[async_trait]
impl TradeSource for NansenClient {
    async fn fetch_page(
        &self,
        address: &str,
        from: DateTime<Utc>,
            ...
            match result {
                Ok(response) if response.status().is_success() => {
                    let value = response
                        .json::<Value>()
                        .await
                        .map_err(|e| format!("NANSEN_INVALID_RESPONSE page={page}: {e}"))?;
                    return Self::decode_page(page, value);
                }
                Ok(response)
                    if matches!(
                        response.status(),
                ...
                {
                    return Err(format!(
                        "NANSEN_AUTH_ERROR page={page} status={}",
                        ...
                        || response.status().is_server_error() =>
                {
                    if attempt == self.max_attempts {
                        return Err(format!(
                            "NANSEN_RETRY_EXHAUSTED page={page} status={}",
                            response.status().as_u16()
                        ));
                    }
                }
                Ok(response) => {
                    return Err(format!(
                        "NANSEN_HTTP_ERROR page={page} status={}",
                        response.status().as_u16()
                    ))
                }
                Err(error) => {
                    if attempt == self.max_attempts {
                        return Err(format!("NANSEN_NETWORK_ERROR page={page}: {error}"));
                    }
                }
            }
